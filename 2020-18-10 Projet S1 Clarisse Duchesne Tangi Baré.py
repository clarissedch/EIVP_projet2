...
#     (theta_id[id[i]-1]).append(theta[i])
#     (temps_id[id[i]-1]).append(temps[i])

def affichage(t1,y,xlabel='t',ylabel='variable',titre='titre'):
    '''affichage de la courbe'''

    plt.plot(t1,y,'+r',linewidth=1,label='lbl2')
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(titre)
    plt.legend(loc="best")
